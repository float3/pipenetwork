...
Matrix::Matrix(size_t size1, size_t size2) : Matrix(size1, size2, 0.0) {}
...
Matrix::Matrix(const std::vector<double> &rowMajorData, size_t size1)
    ...
    if (size1 != 0) {
        ...
        linalghelper::runtime_check(rowMajorData.size() == 0,
                                    "Zero size with non-zero sized data.");
    }

    linalghelper::runtime_check(size1_ * size2_ == rowMajorData.size(), "Inconsistent sizes!");
}

Matrix::Matrix(const std::vector<Vector> &vectorOfRows) : size1_(vectorOfRows.size()), size2_(0) {
    if (size1_ != 0) {
...
double norm(const Vector &vector) {
    return std::sqrt(
        std::inner_product(std::begin(vector), std::end(vector), std::begin(vector), 0.0));
}
...
void updatePermutation(const Matrix &matrix, PermutationVector &permutation, size_t index,
                       double singularTolerance) {
    auto compare = [&](size_t i1, size_t i2) {
        return std::abs(matrix(permutation[i1], index)) < std::abs(matrix(permutation[i2], index));
    };
