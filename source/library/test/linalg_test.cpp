...
TEST_CASE("Matrix_inconsistent_input") {
    std::vector<Vector> inconsistentData{Vector{0.0, 0.0, 0.0}, Vector{0.0, 0.0},
                                         Vector{0.0, 0.0, 0.0}};
...
TEST_CASE("write_matrix") {
    Matrix m({Vector{1.0, -2.0}, Vector{-8.0, 4.0}});
...
TEST_CASE("norm") {
    ...
    Matrix matrix({Vector{1.2, 3.3, 9.1}, Vector{2.9, 8.6, 2.6}, Vector{-4.8, 0.3, -2.1}});

    CHECK(norm(matrix) == Approx(14.553693689232297).epsilon(tolerance));
}

TEST_CASE("solve") {
    Matrix matrix({Vector{5.2, 1.2, 7.3, -2.3}, Vector{8.9, -7.6, -0.2, 3.4},
                   Vector{-5.7, 6.2, -3.4, 7.8}, Vector{9.8, -0.7, 5.4, -2.1}});
...
TEST_CASE("solve_singular") {
    Matrix matrix({Vector{1.0, -1.0}, Vector{-1.0, 1.0}});
...
TEST_CASE("solve_zero_pivot") {
    ...
    Vector expectedSolution{7.57541894507922, -4.311354527696106, -2.8003180763255577,
                            3.456252253714161};
